...
CBrowseFolderDialog::CBrowseFolderDialog(UINT ulFlags,LPCTSTR lpTitle,int nRootFolder)
{
	...
	char temp[256];
	WideCharToMultiByte(CP_ACP, 0, (LPCWSTR)lpTitle, -1, temp, 256, NULL, NULL);
	if( lpTitle != NULL )
	{
		strcpy(m_szTitle,temp);
	}
}
...
{	
	...
	char temp[256];
	WideCharToMultiByte(CP_ACP, 0, (LPCWSTR)lpSelect, -1, temp, 256, NULL, NULL);

	if( lpSelect != NULL )
	{
		strcpy(m_szFirstPath,temp);
	}
...
int CBrowseFolderDialog::GetSelectStr(LPTSTR lpStr, int nMax,BOOL fFullPath)
{
	...
	if( fFullPath == TRUE )
	{
		nStrLen = strlen(m_szSelectedPath);
		if( nStrLen == 0 )
			return 0;
		...
		strncpy(lpStr,m_szSelectedPath,nCopyLen);
		*(lpStr + nCopyLen) = NULL;
	}
	
	else
	{
		nStrLen = strlen(m_szDisplayName);
		if( nStrLen == 0 )
			return 0;
		...
		strncpy(lpStr,m_szDisplayName,nCopyLen);
		*(lpStr + nCopyLen) = NULL;
	}
